...
name       = 'sample_d12_ok';
factor     = 4;
separation = round(300 / factor);
optimize   = true;

%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

load(['./' name '/imp_z_data.mat']);
load(['./' name '/imp_z_data_' num2str(factor) '.mat']);
...
K1(1,1) = K1(1,1) / dataf.factor;
K1(2,2) = K1(2,2) / dataf.factor;
K1(1,3) = K1(1,3) / dataf.factor;
K1(2,3) = K1(2,3) / dataf.factor;

K2(1,1) = K2(1,1) / dataf.factor;
K2(2,2) = K2(2,2) / dataf.factor;
K2(1,3) = K2(1,3) / dataf.factor;
K2(2,3) = K2(2,3) / dataf.factor;
...
save(['./' name '/' results '_' num2str(factor) '.mat'], 'error', 'ok', 'final');
